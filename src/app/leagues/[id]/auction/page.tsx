...
import { Shuffle, Wallet, X, PanelLeftClose, PanelLeftOpen } from 'lucide-react';
...
export default function AuctionPage() {
  ...
  useEffect(() => {
    try {
      const v = localStorage.getItem(`auction_base_${id}`);
  ...
  function startAuction() {
    if (!league) return;
    setPool(league.players.filter(p=>!p.teamId&&!p.isUnsold));
    setUnsoldQueue(league.players.filter(p=>!!p.isUnsold));
    setSoldPlayers(league.players.filter(p=>!!p.teamId));
  ...
  function pickNext() {
    ...
    if(from.length===0){
      ...
      from=[...unsoldQueue];const r=round+1;setRound(r);setUnsoldQueue([]);
      toast.info(`Round ${r} — ${from.length} unsold re-entering`);
    }
    const idx=Math.floor(Math.random()*from.length),picked=from[idx];
    setPool(from.filter((_,i)=>i!==idx));setPhase('picking');
    runSlot(from.map(p=>p.name),picked.name,()=>{setCurrent(picked);setPhase('showing');});
  }
  ...
  async function confirmSold() {
    ...
    try {
      ...
      const ns=[...soldPlayers,{...current,teamId:soldTeamId,soldPrice:price}];
      setSoldPlayers(ns);setCurrent(null);setSoldTeamId('');setSoldPrice('');
      setPhase(ns.length===league!.players.length?'done':'idle');
  ...
  async function markUnsold() {
    if(!current) return;
    await fetch(`/api/leagues/${id}/players/${current.id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({isUnsold:true,teamId:null,soldPrice:null})});
    setUnsoldQueue(prev=>[...prev,current!]);setCurrent(null);setPhase('idle');
  }
  ...
  if(phase==='lobby') return(
    <div className="min-h-screen flex flex-col bg-[oklch(0.085_0.014_260)] text-white relative overflow-hidden">
      ...
      <div className="border-b border-white/8 px-6 py-4 relative z-10 backdrop-blur-xl bg-white/2">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-10 p-8 text-center relative z-10">
        ...
        <div style={{animation:'fadeInUp .55s .25s cubic-bezier(.22,1,.36,1) both'}}><AuctionCTA onClick={startAuction}>Start Auction</AuctionCTA></div>
      </div>
    </div>
  );
    ...
    <div className="min-h-screen flex flex-col bg-[oklch(0.085_0.014_260)] text-white relative overflow-hidden">
      ...
      <div className="flex-1 flex flex-col items-center justify-center gap-8 p-8 text-center overflow-auto">
        ...
        <div className="flex gap-4" style={{animation:'fadeInUp .55s .35s cubic-bezier(.22,1,.36,1) both'}}>
          <Button variant="outline" onClick={startAuction} className="border-white/20 text-white hover:bg-white/10">Restart</Button>
