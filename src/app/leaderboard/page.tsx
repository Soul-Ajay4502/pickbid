...
export default function GlobalLeaderboardPage() {
  const [bids, setBids] = useState<TopBid[]>([]);
  const [loading, setLoading] = useState(true);
  // Drives the entrance animations — flipped on once data is in and painted.
  ...
  // Wait one frame after the list mounts so the height/count-up transitions run.
  useEffect(() => {
    if (!loading) rafRef.current = requestAnimationFrame(() => setRevealed(true));
  }, [loading]);

  const { top, rest } = useMemo(() => ({ top: bids.slice(0, 3), rest: bids.slice(3) }), [bids]);
